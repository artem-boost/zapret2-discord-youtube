...
--filter-tcp=2053,2083,2087,2096,8443 --hostlist-domains=discord.media --lua-desync=fake:blob=tls_clienthello_www_google_com:tcp_ts=-1000:repeats=8 --lua-desync=multisplit:pos=1:seqovl=681:seqovl_pattern=tls_clienthello_www_google_com --new ^
--filter-tcp=443 --hostlist="%LISTS%list-google.txt" --lua-desync=fake:blob=tls_clienthello_www_google_com::tcp_ts=-1000:ip_id=zero:repeats=8 --lua-desync=multisplit:pos=1:seqovl=681:seqovl_pattern=tls_clienthello_www_google_com:ip_id=zero --new ^
--filter-tcp=80,443 --hostlist="%LISTS%list-general.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --lua-desync=fake:blob=tls_clienthello_max_ru:tcp_ts=-1000:ip_id=zero:repeats=8 --lua-desync=multisplit:pos=1:seqovl=654:seqovl_pattern=tls_clienthello_max_ru --new ^
...
--filter-tcp=80,443,%GameFilter% --ipset="%LISTS%ipset-all.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --lua-desync=fake:blob=tls_clienthello_max_ru:tcp_ts=-1000:repeats=8 --lua-desync=multisplit:pos=1:seqovl=654:seqovl_pattern=tls_clienthello_max_ru --new ^

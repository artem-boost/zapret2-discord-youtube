...
--filter-tcp=2053,2083,2087,2096,8443 --hostlist-domains=discord.media --lua-desync=fake:repeats=6:ts:blob=tls_clienthello_www_google_com:tls_mod=none --new ^
--filter-tcp=443 --hostlist="%LISTS%list-google.txt" --lua-desync=fake:blob=tls_clienthello_www_google_com:tcp_md5:tcp_ts:repeats=6 --new ^
--filter-tcp=80,443 --hostlist="%LISTS%list-general.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --lua-desync=fake:repeats=6:ts:blob=tls_clienthello_4pda_to:tls_mod=none --new ^
...
--filter-tcp=80,443,%GameFilter% --ipset="%LISTS%ipset-all.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --lua-desync=fake:repeats=6:ts:blob=tls_clienthello_4pda_to:tls_mod=rnd,sni=www.google.com --new ^
